...
class DeepNeuralDecisionForest(BaseModel):
    def __init__(self, vocab_size, label_size, general_config, config):
        super(DeepNeuralDecisionForest, self).__init__(general_config)
    ...
    def compute_mu(self, flat_decision_p_e):
        ...
        batch_0_indices = torch.range(0, n_batch * self._nleaf - 1, self._nleaf).unsqueeze(1).repeat(1,
                                                                                                     self._nleaf).long()
        ...
        for d in range(1, self._ndepth + 1):
            ...
            batch_complement_indices = torch.LongTensor(
                np.array([[0] * in_repeat, [n_batch * self._nleaf] * in_repeat] * out_repeat).reshape(n_batch,
                                                                                                      self._nleaf))
    ...
    def compute_py_x(self, mu_e, leaf_p_e):
        ...
        for i in range(len(mu_e)):
            py_x_tree = mu_e[i].unsqueeze(2).repeat(1, 1, self._nlabel).mul(
                leaf_p_e[i].unsqueeze(0).repeat(n_batch, 1, 1)).mean(1)
